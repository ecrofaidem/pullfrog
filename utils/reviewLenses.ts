...
export function singleSessionReviewPrompt(agentId: AgentId): string {
  ...
  return `**Single-session review coverage.** Perform all passes yourself in this Codex session. Do not dispatch subagents or start another model session. This is a harness restriction, not a model capability limit. Repository skills that describe a review fleet supply applicable checks; their dispatch instructions do not apply here. Review code without editing it. Continue authorized inspection and checkpoint work without waiting for clarification; when missing information prevents a conclusion, record the limitation.
