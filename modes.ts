...
import { singleSessionReviewPrompt } from "./utils/reviewLenses.ts";
...
2. **checkout**: call \`${t("checkout_pr")}\` — this returns PR metadata, a \`diffPath\`, and a supplemental \`impactPath\` when change-impact extraction is enabled. ${agentId === "codex" ? "Read the table of contents and applicable guidance, then use the single-session checkpoint procedure below to plan and read remaining raw-diff pages. Only after establishing scope, use" : "read the complete raw diff end-to-end, beginning with the TOC and using its file line ranges as your coverage checklist. only after that, use"} \`impactPath\` as an explicitly incomplete list of reference leads; it never replaces raw-diff reading or establishes coverage.
...
3. **triage**: orient yourself on the PR — identify *what kind of thing this is* (domain it touches, seams it crosses, external contracts it depends on, user-facing surfaces it changes). pull as much context as you need to render a confident, well-grounded review: read related files, grep for callers of changed symbols, check tests that exercise the touched paths, fetch related GitHub state. **you are the synthesizer** — never delegate understanding to subagents.
   ...
   only when NOT using Codex and the conventions pass finds no actionable issue, skip the deeper pass and submit a \`No new issues found.\` review per step 7 only when the diff has **no behavioral surface at all** — doc typos, whitespace/formatting, lockfile or generated-code regeneration, a mechanical rename whose only effect is import-path updates. line count is not the signal: a one-line change to auth, money, SQL, a comparison operator, a redirect, or a config default is not trivial.

${agentId === "codex" ? singleSessionReviewPrompt(agentId) : `4. **specialist decision**: after reading the complete diff, name the questions you still cannot answer confidently yourself, and dispatch one \`${REVIEWER_AGENT_NAME}\` specialist per question. a question qualifies only when a specialist could return evidence that **changes your disposition** on the PR — generic requests for another look, extra confidence, or polish do not. most reviews need zero or one; some need several.
...
6. **aggregate & draft**: ${agentId === "codex" ? "after the lens passes and verification, consolidate findings;" : "when specialist results land, merge findings;"} de-dup overlapping findings; trace each finding yourself before accepting it. drop praise, undocumented style preferences (explicit repository requirements remain reviewable), speculative/unverified claims, findings about pre-existing code unrelated to the PR (heuristic: if the finding's root cause lives in lines this PR added or modified, it's in scope; otherwise drop unless the PR plausibly introduced or amplified the regression), and anything not actionable. also drop **bloat-shaped findings** — proposed fixes that would add defensive checks for cases that can't happen, abstractions used once, comments restating obvious code, tests asserting tautologies, or "just-in-case" guards. review passes can recommend unnecessary changes; the bar for an actionable inline comment is sound + correct + elegant. recommending a change that improves only one of the three (or worse, degrades elegance to nominally improve correctness) makes the codebase worse, not better.
...
3. **incremental scope**: if \`incrementalDiffPath\` is present, read it FIRST to see what changed since the last review. this is a range-diff that isolates the net changes, filtering out base branch noise. ${agentId === "codex" ? "Then inspect the full-diff TOC and current guidance, and call review_checkpoint action=plan before rereading the full patch. Follow its analysisScope and remaining read ranges: incremental scope reuses verified prior coverage; full scope requires finishing outstanding full-PR work. If no incremental diff exists, review the full PR." : "then read the authoritative full diff end-to-end, beginning with the TOC and using its line ranges as your coverage checklist. if no incremental diff is present, start with the full-diff TOC, determine what changed since Pullfrog's most recent review, and complete the raw-diff read."} only after establishing that authoritative scope and completing raw-diff coverage, use \`impactPath\` as an explicitly incomplete list of reference leads; it never replaces raw-diff reading or establishes coverage.
...
${agentId === "codex" ? singleSessionReviewPrompt(agentId) : `6. **specialist decision**: after covering the incremental and full diffs, name the questions about the new changes that you still cannot answer confidently yourself, and dispatch one \`${REVIEWER_AGENT_NAME}\` specialist per question. a question qualifies only when a specialist could return evidence that **changes your disposition** on the PR — generic requests for another look, extra confidence, or polish do not. most incremental reviews need zero or one, especially thread-reply re-reviews; some need several.
...
8. **aggregate, draft, self-critique**: ${agentId === "codex" ? "When analysisScope is full, finish and report previously unreviewed full-PR concerns, deduplicating prior feedback; the new-delta-only filters below apply only when analysisScope is incremental. Consolidate findings from your passes;" : "merge findings (yours + output from every specialist you dispatched);"} de-dup overlaps; trace each finding yourself. drop praise, undocumented style preferences (explicit repository requirements remain reviewable), speculative/unverified claims, findings about pre-existing code unrelated to the new commits, anything not actionable, and anything that re-states prior review feedback (heuristic: if the finding's root cause lives in lines the *new commits* added or modified, it's in scope; otherwise drop). also drop **bloat-shaped findings** — proposed fixes that would add defensive checks for cases that can't happen, abstractions used once, comments restating obvious code, tests asserting tautologies, or "just-in-case" guards. review passes can recommend unnecessary changes; the bar for an actionable inline comment is sound + correct + elegant. recommending a change that improves only one of the three (or degrades elegance to nominally improve correctness) makes the codebase worse, not better. To compute "lines the new commits added or modified": if \`incrementalDiffPath\` from step 2 is present, use it directly. Otherwise, take the prior Pullfrog review's \`commit_id\` (returned alongside each entry from \`${t("list_pull_request_reviews")}\` in step 4) and run \`git diff <prior-review-sha>..HEAD\` to isolate the lines added since that review.
...
10. Submit — every run must end with EXACTLY ONE of \`${t("create_pull_request_review")}\` (substantive review) or \`${t("report_progress")}\` (no-review acknowledgement). do NOT call \`create_issue_comment\` for review output.
   ...
   Follow these rules:
   - ${agentId === "codex" ? "Finish the read ranges and passes reported by review_checkpoint before final verification. If blocked after appropriate recovery, record an incomplete checkpoint and publish the partial findings without approval." : "note: the first create_pull_request_review submission may error with a one-time diff-coverage nudge listing unread TOC regions. Read the relevant ranges and retry the submission."}
